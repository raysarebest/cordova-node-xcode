...
var FILETYPE_BY_EXTENSION = /** @type {const} */ ({
    ...
    /** @type {Object.<import("./xcode-types/xcfiletreeobject").XcodeFileType, string>} */
    GROUP_BY_FILETYPE = {
    ...
    /** @type {Object.<import("./xcode-types/xcfiletreeobject").XcodeFileType, import("./xcode-types/xcfiletreeobject").XcodeFileEncoding>} */
...
function defaultExtension(fileRef) {
    ...
    for(var extension in FILETYPE_BY_EXTENSION) {
        if(FILETYPE_BY_EXTENSION.hasOwnProperty(unquoted(extension)) ) {
             if(FILETYPE_BY_EXTENSION[unquoted(extension)] === unquoted(filetype || null) )
                 return extension;
        }
    }
}
...
function defaultEncoding(fileRef) {
    var filetype = "lastKnownFileType" in fileRef ? fileRef.lastKnownFileType : fileRef.explicitFileType,
        encoding = ENCODING_BY_FILETYPE[unquoted(filetype || null)];
...
/**
 * Configuration options available (but not required) for a {@link pbxFile} that can be used as source for a compiler
 * 
 * @typedef {Object} PBXCompiledFileOptions
 * @property {string} compilerFlags A space-separated list of options to pass to the relevant compiler when building the file
 */

/**
 * Configuration options available (but not required) for a {@link pbxFile}
 * 
 * @typedef {PBXBaseFileOptions | (PBXBaseFileOptions & PBXFileTypeOptions) | (PBXBaseFileOptions & PBXFrameworkFileOptions) | (PBXBaseFileOptions & PBXFileTypeOptions & PBXFrameworkFileOptions) | PBXFileTypeOptions | (PBXFileTypeOptions & PBXFrameworkFileOptions) | PBXFrameworkFileOptions | PBXCompiledFileOptions | (PBXCompiledFileOptions & PBXBaseFileOptions) | (PBXCompiledFileOptions & PBXFileTypeOptions) | (PBXCompiledFileOptions & PBXFrameworkFileOptions) | (PBXCompiledFileOptions & PBXBaseFileOptions & PBXFileTypeOptions) | (PBXCompiledFileOptions & PBXBaseFileOptions & PBXFrameworkFileOptions) | (PBXCompiledFileOptions & PBXFileTypeOptions & PBXFrameworkFileOptions) | (PBXCompiledFileOptions & PBXBaseFileOptions & PBXFileTypeOptions & PBXFrameworkFileOptions)} PBXFileOptions
 */
 ...
 * @property {import("./xcode-types/xcfiletreeobject").XcodeFileEncoding} [fileEncoding] An identifier for the text encoding that the file should use. Will be `undefined` if a file encoding isn't specified and can't be detected by this library. See {@link XcodeFileEncoding} for all options
 ...
 * @property {import("./xcode-types/xcode").XcodeProjectObjectReference<import("./xcode-types/pbxtarget").PBXTarget>} [target] The identifier of the target the file is included in
 * @property {import("./xcode-types/xcode").XcodeProjectObjectReference<import("./xcode-types/xcfiletreeobject").PBXFileReference>} [fileRef] The identifier of the Xcode-internal metadata object that describes how it references the file on-disk. May not be present if the metadata object represents a file that was removed
 * @property {boolean} [plugin] Indicates whether the file is or is involved in a Cordova plugin
 */
 ...
 * @property {import("./xcode-types/xcfiletreeobject").XcodeFileEncoding} [defaultEncoding] An identifier for the text encoding that the file should use. Will be `undefined` if a file encoding isn't specified and can't be detected by this library. See {@link XcodeFileEncoding} for all options
 */
 ...
 * 
 * @typedef {(new (filepath: string, opt: PBXCompiledFileOptions & PBXFrameworkFileOptions & PBXExplicitFileTypeOptions) => (pbxCoreFile & pbxExplicitTypeFile & pbxCustomCompiledFile & pbxFrameworkFile)) &
 *           (new (filepath: string, opt: PBXCompiledFileOptions & PBXFrameworkFileOptions & PBXInferredFileTypeOptions) => (pbxCoreFile & pbxInferredTypeFile & pbxCustomCompiledFile & pbxFrameworkFile)) &
 *           (new (filepath: string, opt: PBXCompiledFileOptions & PBXFrameworkFileOptions) => (pbxCoreFile & pbxInferredTypeFile & pbxCustomCompiledFile & pbxFrameworkFile)) &
 *           (new (filepath: string, opt: PBXCompiledFileOptions & PBXExplicitFileTypeOptions) => (pbxCoreFile & pbxExplicitTypeFile & pbxCustomCompiledFile)) &
 *           (new (filepath: string, opt: PBXCompiledFileOptions & PBXInferredFileTypeOptions) => (pbxCoreFile & pbxInferredTypeFile & pbxCustomCompiledFile)) &
 *           (new (filepath: string, opt: PBXCompiledFileOptions) => (pbxCoreFile & pbxInferredTypeFile & pbxCustomCompiledFile)) &
 ...
 *           (new (filepath: string, opt: {}) => (pbxCoreFile & pbxInferredTypeFile))} pbxFileConstructor
 */
...
module.exports = function (filepath, opt = {}) {
    ...
    /** @type {string} */
    // @ts-ignore
    this.fileRef = /** @type {string} */ (undefined); // Assigned by specific pbxProject functions later, necessary to silence errors now
    this.basename = path.basename(filepath);

    // Many properties coming up get created or destroyed based on the ultimate concrete return type, which can't be expressed in JSDoc
    // TypeScript ignore directives are all related to that fact
    ...
    if ("weak" in opt && opt.weak === true)
        /** @type {{ATTRIBUTES?: import("./xcode-types/pbxbuildfile").XcodeBuildFileSettingAttributes, COMPILER_FLAGS?: string | number}} */
        // @ts-ignore
